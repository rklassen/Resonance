...
- `property` is a constrained operator with a unitary input and a non-fallible,
  typed value output.
- `v0.8` added dynamic edge and node weight operators in form `-(@op_id)->`.
...
- Typed grouped edges from `v0.4` are part of `v0.8`.
- Edge families are block-scoped.
- Edge weight is optional and may be a scalar, list, matrix, bytestream
  reference, or operator reference; see `Edge weight embeddings (v0.6)`
  and `Dynamic edge weights via operator refs (v0.8)`.
...
- Brackets `[` and `]` are not used in edge weights. A length-1 vector is
  equivalent to a scalar in the data model and is serialized as a scalar in
  the codec.
...
### Dynamic edge weights via operator refs (v0.8)

A bare `@id` inside the weight parens (no slice) refers to an `operator`
node defined in `nodes`. The runtime evaluates the operator to produce
the weight at use time. The operator node definition is unchanged from
v0.6; the new behavior is purely at the edge weight grammar.

Resolution rule by suffix inside the parens:

- `@id` bare -> operator ref. The id must resolve to `node_type: operator`.
- `@id ..len` or `@id +offset..len` -> bytestream slice (v0.6).

The format mark on the arrow records what the runtime should coerce the
operator's return value to:

- `-(@scorer)->` raw return (whatever shape the operator declares)
- `-(@scorer)s->` snorm-coerced return
- `-(@scorer)u->` unorm-coerced return
- `-(@scorer)h->` byte-interpreted return

Validation rules:

- A bare `@id` in weight position must resolve to a node of node_type
  `operator` or `stream`. Other kinds are rejected.
- If the target node_type is `stream`, the slice form `..len` is required.
- If the target node_type is `operator`, the slice form is rejected.
- The operator's `out:` field must not be `None`; otherwise the weight
  has nothing to evaluate to.
- Format marks on operator-ref weights are not validated at parse time
  (the value is dynamic); they are coercion hints for the runtime.

Internal data model mapping (for implementers):

- bare operator ref -> `OpRef(NodeId, encoding)`
...
Any node entry may carry an optional `weight:` field. The value uses the same
grammar and validation rules as edge weights, without the surrounding arrow.

Examples:

```snap
function LandXmlWriter.write$32gp {
 in: { path: FileTypeLandXml, surface: FileTypeLandXml, },
 out: None,
...
- Timestamps use ISO 8601 UTC without seconds and with terminal `Z`.
- Strings use single quotes when quoting is required.
